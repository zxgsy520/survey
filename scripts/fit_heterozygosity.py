...
def read_histo(file, estimate):

    r = {}

    for line in open(estimate):

        line = line.strip()
        
        if line.startswith("Total kmer individuals(lowfreq excluded):"):
            i = int(line.split(":")[1])
            break
        else:
            continue
...
def add_args(parser):
    ...
    parser.add_argument("-e", "--estimate", required=True, help="")
...
contact:  %s <%s>\
    ...
    data, n, i, k = read_histo(args.histo, args.estimate)
